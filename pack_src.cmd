setlocal
set dst="SageThumbs_{version}_Source.7z"
...
del /q %dst% 2>nul
%zip% a -y -mx=9 -r -x!.svn -x!setup\*.exe -x!Win32 -x!x64 -x!ipch -x!*.po_ -x!*.7z -x!*.log -x!*.bak -x!*.tmp -x!*.sdf -x!*.suo -x!*.ncb -x!*.user -x!*.opensdf %dst% .\